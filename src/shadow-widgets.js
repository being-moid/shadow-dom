// Add safe registration helper at the top
function safeDefineComponent(name, component) {
    if (!customElements.get(name)) {
        try {
            customElements.define(name, component);
        } catch (error) {
            console.warn(`Warning: ${name} already registered`);
        }
    }
}

// Import components with their classes
import { FloatingButton } from './components/FloatingButton.js';
import { ServiceTable } from './components/ServiceTable.js';
import { Modal } from './components/Modal.js';
import { DataViewModal } from './components/DataViewModal.js';
import { CoverageVerification } from './components/CoverageVerification.js';
import { PriorAuthClaimManagement } from './components/PriorAuthClaimManagement.js';
import { PriorAuthWidget } from './components/PriorAuthWidget.js';
import { PriorAuthGridModal } from './components/PriorAuthGridModal.js';
import { PriorAuthGrid } from './components/PriorAuthGrid.js';
import { PriorAuthFormWidget } from './components/prior-auth-form/PriorAuthFormWidget.js';

// Register all components safely
safeDefineComponent('floating-button', FloatingButton);
safeDefineComponent('service-table', ServiceTable);
safeDefineComponent('modal-component', Modal);
safeDefineComponent('data-view-modal', DataViewModal);
safeDefineComponent('coverage-verification', CoverageVerification);
safeDefineComponent('prior-auth-claim-management', PriorAuthClaimManagement);
safeDefineComponent('prior-auth-widget', PriorAuthWidget);
safeDefineComponent('prior-auth-grid-modal', PriorAuthGridModal);
safeDefineComponent('prior-auth-grid', PriorAuthGrid);
safeDefineComponent('prior-auth-form-widget', PriorAuthFormWidget);
...
// Create the ShadowWidgets API with factory methods
const ShadowWidgets = {
    // Component classes
    FloatingButton,
    ServiceTable,
    ...
    createPriorAuthFormWithButton(options = {}) {
        const btn = this.createFloatingButton({
            ...options,
            position: 'bottom-right-4',
            type: 'prior-auth-form'
        });

        const container = document.createElement('div');
        container.id = 'prior-auth-form-container';
        container.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
        ...
        const priorAuthForm = document.createElement('prior-auth-form-widget');
        priorAuthForm.style.cssText = `
            margin: 0 auto;
            width: 100%;
            max-width: 1200px;
            height: calc(100vh - 4rem);
        ...
        // Move the content into the wrapper
        while (priorAuthForm.firstChild) {
            contentWrapper.appendChild(priorAuthForm.firstChild);
        }
        ...
        modalWrapper.appendChild(contentWrapper);
        modalWrapper.appendChild(actionContainer);
        priorAuthForm.appendChild(modalWrapper);
        container.appendChild(priorAuthForm);
        document.body.appendChild(container);
        ...
        const showPopup = () => {
            ...
            requestAnimationFrame(() => {
                container.style.opacity = '1';
                priorAuthForm.style.opacity = '1';
                priorAuthForm.style.transform = 'translateY(0)';
            });
            document.body.style.overflow = 'hidden';
        };
        ...
        const hidePopup = () => {
            container.style.opacity = '0';
            priorAuthForm.style.opacity = '0';
            priorAuthForm.style.transform = 'translateY(20px)';
            setTimeout(() => {
                container.style.display = 'none';
                document.body.style.overflow = '';
            }, 300);
        ...
        priorAuthForm.addEventListener('close', hidePopup);
        ...
        return { container, btn, priorAuthForm, cleanup };
    }
};

// Make sure ShadowWidgets is available globally
